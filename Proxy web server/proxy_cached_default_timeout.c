...
handler_t *Signal(int signum, handler_t *handler) 
{
    ...
    action.sa_flags = SA_RESTART; /* restarts syscalls*/
 ...
 **********************************************************************/
/*
 * rio_readn - robustly reads n bytes (unbuffered)
 */
/* $begin rio_readn */
...
/*
 * open_clientfd - opens connection to server at <hostname, port> 
 *   and returns a socket descriptor ready for reading and writing.
...
int open_clientfd(char *hostname, int port) 
{
	...
	return -1; /* checks errno for cause of error */

    /* Filling in the server's IP address and port */
...
int open_listenfd(int port) 
{
    ...
    /* Making it a listening socket ready to accept connection requests */
...
int add_to_cache(char * data, int site_size, char* uri) {
	if (site_size + sizeof(cache_object) <= MAX_OBJECT_SIZE)
	{
		...
		pthread_rwlock_wrlock(&lock); /* locks on the resource*/
		/* Freeing enough space for the new site in cache*/
		while(cache_size + site_size + sizeof(cache_object) >= MAX_CACHE_SIZE ){
			remove_from_cache();
		}
		/* Adds the site node to the start of the cache list*/
...
/*
	remove_from_cache - adds a cache_object to the cache
*/
void remove_from_cache(){
	...
	if( head != NULL) {
		...
		/* frees required node from the list*/
		if(temp == head) {
		...
		/* Changes cache size(free temp node) */
...
cache_object * site;
	...
	/* Reads each line of the header request*/
	while(rio_readlineb(&client_rio, buffer1, MAXLINE) && 
		strcmp(buffer1, "\r\n")){
		/*if first line, retrieves method and uri */
		dbg_printf("---------------------- %d\n", count);
	...
	/* Adds new line at the end of request*/
	strcat(request, "\r\n");
...
printf("\n url is not in cache\n"); 
	...
	/* Finds uri in cache and writes to clientfd */
...
printf("this is a else printf");
	...
	/* Reading response from Server, sending to client and adds to cache */
