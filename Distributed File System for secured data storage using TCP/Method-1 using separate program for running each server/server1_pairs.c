...
/* ***************************************************************************** */
 ...
 if (client_fd<0)              //if there are negative requests, then error
            error ("accept() error");
        else                        //otherwise forking to handle multiple simultaneous requests
        {
            if ( fork()==0 )
            {
...
{
char *directory = "/home/swaminath/Downloads/split/pairs/server1/Alice";
chdir (directory); 
printf("\n We are under Alice folder ");
}
else if(strcmp(server_nuser,"Bob")==0)
{
char *directory = "/home/swaminath/Downloads/split/pairs/server1/Bob";
chdir (directory); 
printf("\n We are under Bob folder ");
}

char user_server[30];char buffer_pass[20];
...
int str_1,str_2;
char folder[250]; char verify[] = "password";//password string for decrytping the hash
...
recv(client_fd,put_sub,8,0);   //receiving info about the name of server folder to store data
printf("\n put_sub is %s", put_sub);
...
{
printf("\n Password key mismataches, data can't be decrypted");
}
...
char *directory1 = "/home/swaminath/Downloads/split/pairs/server1";
chdir (directory1); 
}
...
char *directory2 = "/home/swaminath/Downloads/split/pairs/server1";
chdir (directory2); 
}
...
char *directory3 = "/home/swaminath/Downloads/split/pairs/server1";
chdir (directory3); 
}
